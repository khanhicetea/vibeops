 ...
 * for stable compose-relative paths.
 */
export async function materializeDockerAssets(
  ...
  await platform.fs.mkdirp(helpersDir);
  await platform.fs.mkdirp(nginxContext);
  await platform.fs.mkdirp(phpContext);
  ...
  await materializePaths(platform, [...HELPER_FILES], platform.paths.paths.root, "");

  // Copy stack helpers into PHP build context for image COPY helpers/
