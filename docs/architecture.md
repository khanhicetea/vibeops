...
- **Logical dump durability:** backups stream into a private partial file and are promoted atomically to a unique finalized `*.sql` path only after a successful non-empty dump. Partials are excluded from listing/retention. Restore streams the dump into `mysql` (not fully buffered in the Python process) and may overwrite objects; object-level restore is not atomic.
