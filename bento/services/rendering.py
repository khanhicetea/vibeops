...
from bento.utils.template import render_template_text as render_bento_template_text

from bento.utils.errors import die
from bento.os.fsutil import write_text
from bento.utils.paths import GENERATED_HEADER, GENERATED_NOTICE, PHP_FPM_GENERATED_HEADER, rel

def render_template_text(text: str, values: dict[str, Any]) -> str:
    return render_bento_template_text(text, values)
...
def content_looks_generated(path: Path) -> bool:
    """True when a file carries the bento generated notice (managed marker)."""
    try:
