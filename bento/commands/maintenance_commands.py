...
_CRON_BEGIN = "# BEGIN bento maintance"
_CRON_END = "# END bento maintance"
...
def _cron_command() -> str:
    ...
    return f"cd {shlex.quote(root)} && {shlex.quote(manage)} maintance"
...
def setup_cron(schedule: str) -> None:
    ...
    if len(begin_indexes) != len(end_indexes) or len(begin_indexes) > 1:
        die("Current crontab has a malformed bento maintance block; repair it manually")
    if begin_indexes:
...
def cmd_maintance(args: argparse.Namespace) -> None:
    action = getattr(args, "maintance_action", None) or "run"
...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    maintance = subparsers.add_parser(
        "maintance",
        aliases=["maintenance", "maintaince"],
        help="Run stack maintenance jobs or register them in host cron",
    )
    actions = maintance.add_subparsers(dest="maintance_action")
    ...
    run_now.set_defaults(func=cmd_maintance)
    setup = actions.add_parser("setup-cron", help="Register ./manage.py maintance in the current user's crontab")
    setup.add_argument("--schedule", default=_DEFAULT_SCHEDULE, help="Five-field host cron schedule")
    setup.set_defaults(func=cmd_maintance)
    maintance.add_argument("--force", action="store_true", help="Force log rotation regardless of size")
    maintance.set_defaults(func=cmd_maintance)
