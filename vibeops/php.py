...
from vibeops.env import default_fpm_profile, default_mysql_service, default_php_version, fpm_pool_template_values, stack_env, validate_fpm_profile
from vibeops.errors import die, info
from vibeops.fsutil import mkdir
from vibeops.mysql import apply_app_mysql_metadata, create_mysql_user
from vibeops.paths import DOCROOT_NAME, HOME_DIR, LEGACY_PHP_VERSIONS_DIR, PHP_LOG_DIR, PHP_SOCKET_DIR, PHP_TEMPLATE_DIR, PHP_VERSIONS_DIR, RenderContext, rel
from vibeops.process import run, service_running
from vibeops.rendering import write_template
from vibeops.state import allocate_uid, read_uid_from_env, upsert_timestamp
from vibeops.validation import APP_NAME_RE, MYSQL_SERVICE_RE, PHP_VERSION_RE, validate, validate_php_entrypoint, validate_public_dir
...
def ensure_app_identity(app_name: str, php_version: str, db: dict[str, Any], *, uid: int | None = None, public_dir: str | None = None, fpm_profile: str | None = None, no_mysql: bool = False, mysql_password: str | None = None, mysql_service: str | None = None, no_reload: bool = False) -> dict[str, Any]:
    ...
    from vibeops.cron_runtime import cron_dir_for
