...
from vibeops.services.cron_runtime import app_crontab_for, system_crontab_for
from vibeops.services.php import php_runner_service_for, php_service_for
from vibeops.utils.env import default_php_version
from vibeops.utils.errors import die, info, warn
from vibeops.utils.paths import DOCROOT_NAME, GENERATED_NOTICE, RUNNER_RUNTIME_DIR, RenderContext
from vibeops.utils.validation import APP_NAME_RE, JOB_RE, PHP_VERSION_RE, validate, validate_cron_workdir
...
def runner_program_path(version: str, ctx: RenderContext | None = None) -> Path:
    return runner_programs_dir_for(version, ctx) / "vibeops.conf"
...
def _common_program_lines(app_name: str, workdir: str) -> list[str]:
    return [
        ...
        (
            ...
            f'COMPOSER_HOME="/home/{app_name}/.composer",VIBEOPS_APP="{app_name}"'
        ),
...
def validate_runner(db: dict[str, Any], version: str) -> None:
    ...
    if service == runner:
        ...
        run(compose_command("exec", "-T", service, "supervisorctl", "-c", "/etc/vibeops/supervisord.conf", "reread"))
    else:
...
def reconcile_runner(db: dict[str, Any], version: str) -> None:
    ...
    if not service_running(service):
        ...
        return
    run(compose_command("exec", "-T", service, "supervisorctl", "-c", "/etc/vibeops/supervisord.conf", "reread"))
    run(compose_command("exec", "-T", service, "supervisorctl", "-c", "/etc/vibeops/supervisord.conf", "update"))
    # Crontab contents are external to Supervisor config, so reload schedulers
    # even when reread reports no program-level changes.
    run(compose_command("exec", "-T", service, "supervisorctl", "-c", "/etc/vibeops/supervisord.conf", "signal", "USR2", "system-cron"))
    cron_apps = sorted({
        str(cron.get("app"))
        for cron in db.get("crons", {}).values()
        if isinstance(cron, dict) and str(cron.get("php_version") or default_php_version()) == version
    ...
    for app_name in cron_apps:
        result = run(
            compose_command("exec", "-T", service, "supervisorctl", "-c", "/etc/vibeops/supervisord.conf", "signal", "USR2", cron_target(app_name)),
            check=False,
            capture=True,
        )
