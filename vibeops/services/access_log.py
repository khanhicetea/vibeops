...
from vibeops.utils.paths import NGINX_ACCESS_LOG_DIR, ROOT, rel
from vibeops.os.process import docker_available, run, service_running
...
def _finish_archives(app_archives: list[tuple[str, Path]]) -> None:
    ...
    for app_name, archive in app_archives:
        try:
            ...
            info(f"Compressed vibeops/{rel(gz)}")
        except OSError as exc:
            warn(f"could not compress vibeops/{rel(archive)}: {exc}")
...
def rotate_app_access_log(app_name: str, *, force: bool = False) -> bool:
    ...
    info(f"Rotated vibeops/{rel(live)} -> vibeops/{rel(archive)} ({size} bytes)")
...
def run_goaccess_analyze(
    ...
    *,
    ...
    if not files:
        die(
            f"No access log files for {app_name} under vibeops/{rel(NGINX_ACCESS_LOG_DIR)}. "
