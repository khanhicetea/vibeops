...
from vibeops.utils.paths import CUSTOM_DIR, NGINX_TEMPLATE_DIR, PHP_TEMPLATE_DIR, rel
...
def template_sha256(path: Path) -> str:
    try:
        ...
        die(f"Cannot read template vibeops/{rel(path)}: {exc}")
...
def selected_template_path(app: dict[str, Any], target: str) -> Path:
    ...
    if not path.is_file():
        die(f"Missing custom {target} template: vibeops/{rel(path)}")
    return path
...
def install_custom_template(app_name: str, target: str, *, force: bool = False) -> tuple[Path, bool]:
    ...
    if destination.exists() and not destination.is_file():
        die(f"Custom {target} source is not a regular file: vibeops/{rel(destination)}")
    if destination.exists() and not force:
        return destination, False
    if not source.is_file():
        die(f"Missing upstream template: vibeops/{rel(source)}")
    ...
    header = (
        f"{marker} VIBEOPS CUSTOM APP TEMPLATE. This source is user-owned and survives renders.\n"
        f"{marker} {contract}\n"
    )
