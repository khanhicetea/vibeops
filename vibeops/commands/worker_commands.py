...
from vibeops.commands.app_commands import resolve_app_php_version
from vibeops.os.process import run, service_running
from vibeops.services.compose import compose_command
from vibeops.services.php import php_runner_service_for
from vibeops.services.runner import normalize_worker, worker_target
from vibeops.services.state import cron_state_lock, load_db, save_db, upsert_timestamp
from vibeops.ui.table import print_table
from vibeops.utils.errors import die, info
from vibeops.utils.paths import DOCROOT_NAME
from vibeops.utils.validation import APP_NAME_RE, JOB_RE, validate, validate_cron_workdir
...
def cmd_worker_control(args: argparse.Namespace) -> None:
    ...
    run(compose_command("exec", "-T", service, "supervisorctl", "-c", "/etc/vibeops/supervisord.conf", action, target))
    if action != "status":
