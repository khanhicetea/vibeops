...
from vibeops.services.access_log import (
...
def cmd_app_access_log(args: argparse.Namespace) -> None:
        ...
        if files:
            for path in files:
                try:
                    ...
                    size = 0
                info(f"  vibeops/{rel(path)} ({size} bytes)")
        else:
    ...
    if enabled:
        info(f"Logs: vibeops/{rel(live_access_log_path(app_name))}")
